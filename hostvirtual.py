# -*- coding: utf-8 -*-
'''
HostVirtual Cloud Module
===================

The HostVirtual cloud module is used to control access to the HostVirtual VPS system
...
.. code-block:: yaml
    ...
    # HostVirtual account api key
    HOSTVIRTUAL.apikey: JVkbSJDGHSDKUKSDJfhsdklfjgsjdkflhjlsdfffhgdgjkenrtuinv


Using the new format, set up the cloud configuration at
 ``/etc/salt/cloud.providers`` or ``/etc/salt/cloud.providers.d/hostvirtual.conf``:

.. code-block:: yaml

    my-hostvirtual-config:
      # HostVirtual account api key
...
def __virtual__():
    '''
    Set up the libcloud functions and check for HostVirtual configurations.
    '''
    if get_configured_provider() is False:
        log.debug(
            'There is no HostVirtual cloud provider configuration available. Not '
            'loading module.'
        )
        return False

    log.debug('Loading HostVirtual cloud module')
    return True
...
def get_configured_provider():
    ...
    return config.is_provider_configured(
        __opts__,
        __active_provider_name__ or 'hostvirtual',
        ('apikey',)
    )
...
def create(vm_):
    ...
    try:
        # pop off the first 3 since fucking hv driver
